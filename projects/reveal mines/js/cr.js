...
function renderBoard(board) {
    ...
    for (var i = 0; i < gLevel.SIZE; i++) {
        ...
        for (var j = 0; j < gLevel.SIZE; j++) {
            ...
            strHTML += `<td id ="cell-${i}-${j}" class="${className}" 
            onmousedown="cellClicked(event,this.id)" > ${img} </td>`;
        }
        strHTML += '</tr>'
    }
    strHTML += '</tbody></table>';
...
function cellClicked(ev, id) {
    ...
    if (gGame.isOn) {
        ...
        var j = +id.charAt(7);
        var cell = gBoard[i][j];
        //CR: when right clicking a cell it shouldn't open the menu of right click
        if (ev.button === 2) {
            if (!cell.isShown) {
                if (!cell.isMarked) {
                    ///put flag image  
...
//working recursivly
//CR: in some cases it doesn't reveal all empty cells
...
//CR: in order to win a game all mines should be flagged 
//and other cells should be revealed
function checkWin() {
